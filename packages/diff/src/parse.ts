import type { DiffLine, FileStatus, Hunk, LineType, PullRequestFile } from "@folio/types";
import { FILE_STATUS, LINE_TYPE } from "@folio/types";
import parseDiff from "parse-diff";

// ─── parse-diff shape (the npm package returns these untyped-ish structures) ──
...
function getFilePath(file: ParseDiffFile): string {
  if (file.deleted) {
    if (!file.from) {
      throw new Error("Deleted file is missing 'from' path");
    }
    return file.from.replace(/^a\//, "");
  }
  const filePath = file.to ?? file.from;
  if (!filePath) {
    throw new Error("File is missing both 'to' and 'from' paths");
  }
...
/** Detect symlink patches via mode `120000` on any mode/index header line. */
...
/**
 * Split a multi-file unified diff into per-file raw patch segments, on
 * `diff --git ` line boundaries, preserving each file's header lines.
 */
...
/**
 * Parse a unified diff string into `PullRequestFile[]`, preserving per-file
 * headers and two-column old/new line numbers. Wraps the `parse-diff` package.
 * Empty / whitespace-only input → `[]`.
 */
export function parseUnifiedDiff(diffOutput: string): PullRequestFile[] {
...
/** Alias matching the contract name `parseGitDiff` from the task brief. */
