...
const listByInstallation = vi.fn(async () => [
  { id: "r1", owner: "KMGeon", name: "Folio", fullName: "KMGeon/Folio" },
]);
...
describe("DashboardFacade", () => {
  it("lists live open PRs with DB review status merged in", async () => {
    ...
    expect(payload.metrics).toEqual({ ready: 1, processing: 1, installedRepos: 1 });
    expect(payload.repos).toEqual([{ fullName: "KMGeon/Folio", openPrCount: 2 }]);
