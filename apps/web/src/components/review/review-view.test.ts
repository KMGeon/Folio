...
describe("ReviewView source", () => {
  it("matches the compact review density reference", () => {
    ...
    expect(topBarSource).toContain("border-b-2 px-2.5 py-1.5 text-xs");
    expect(source).toContain("overflow-y-auto px-4 py-4 md:px-6 md:py-5");
  });
  ...
  it("uses Approach A contrast for chapter summary and review questions", () => {
    ...
    // Focus cards: amber fill only (no inset left rail / no info blue).
    expect(chapterPanelSource).toContain("bg-warning/20");
    expect(chapterPanelSource).toContain("bg-warning/12");
    expect(chapterPanelSource).not.toContain("shadow-[inset_3px_0_0_0]");
