  ...
  const changeRepository = async (repository: RepositorySummary, enabled: boolean) => {
    ...
    try {
      ...
      setErrorsByRepositoryId((current) => ({
        ...current,
        [repository.id]: "저장소 설정을 변경하지 못했습니다.",
      }));
    } finally {
      ...
      <td className="px-3 py-2">
        <div className="flex flex-col items-end gap-1">
          <Switch
            ...
            label={`${repository.fullName} Folio 리뷰 ${checked ? "끄기" : "켜기"}`}
            onCheckedChange={onCheckedChange}
          />
          {disabledReason ? (
            <span className="max-w-72 text-right text-muted-foreground">{disabledReason}</span>
          ) : null}
          {error ? (
            <span role="alert" className="text-right text-destructive">
              {error}
            </span>
          ) : null}
        </div>
